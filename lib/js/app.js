...
(function () {
  ...
  window.onload = function () {
    ...
    function getCanvas(city, timeOffset) {
      ...
      this.ctx = this.canvas.getContext("2d"), this.radius = this.canvas.height / 2, this.transl = function () {
        _this.ctx.translate(_this.radius, _this.radius);

        _this.radius = _this.radius * 0.90; //Changes this to adjust clock size
      };
    } //New York time
    ...
    function drawTime(ctx, radius, city) {
      ...
      ctx.fillText(mid.toString(), 0, radius * 0.60); //hour

      hour = hour % 12;
      hour = hour * Math.PI / 6 + minute * Math.PI / (6 * 60) + second * Math.PI / (360 * 60);
      drawHand(ctx, hour, radius * 0.5, radius * 0.02, city); //minute

      minute = minute * Math.PI / 30 + second * Math.PI / (30 * 60);
      drawHand(ctx, minute, radius * 0.8, radius * 0.02, city); // second
